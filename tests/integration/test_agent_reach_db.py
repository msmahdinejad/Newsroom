"""Social collection PostgreSQL integration tests — real DB, no MagicMock sessions.

Tests per the module contract section 13:
...
def db(engine):
    ...
    session = factory()
    # Clean Social collection-specific rows before each test. Order matters for FKs.
...
def test_alembic_at_gate5_revision(engine):
    ...
    # Production advances to 0009_gate6_source_inventory — Social collection tables still exist
    assert row[0] in (
