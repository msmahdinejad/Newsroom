...
- **Logs**: Token never logged. `redact_token()` shows only `1234...4567` format.
...
### Authorized User IDs
- Stored in environment variable `TELEGRAM_AUTHORIZED_USERS`
...
```
- Verified: `test_redact_token_no_leak` — middle portion never visible
- Verified: `test_redact_token_short` — short tokens fully redacted
- Used in bot startup log: `Bot identity: @{username} (1234...4567)`
...
- `test_redact_token_no_leak` — middle portion not visible
- `test_env_example_has_empty_token` — `TELEGRAM_BOT_TOKEN=` (empty)
- `test_env_example_has_empty_authorized_users` — `TELEGRAM_AUTHORIZED_USERS=` (empty)
